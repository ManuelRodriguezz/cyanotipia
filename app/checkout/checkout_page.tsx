...
import { ArrowLeft, CreditCard, MessageCircle, ShieldCheck, Truck, Banknote } from "lucide-react";
import { motion } from "framer-motion";
import CheckoutNavbar from "../components/CheckoutNavbar";

const WA_NUMBER = "5491132503050";
...
export default function CheckoutPage() {
  ...
  const [payMethod, setPayMethod] = useState<"mp" | "wa" | "efectivo">("mp");
  ...
  const buildWAMessage = (extra = "") => {
    const lineas = items
      .map((i) => `• ${i.name} x${i.qty} = $${(i.price * i.qty).toLocaleString("es-AR")}`)
      .join("\n");
    return `¡Hola cyanotipia! Quiero realizar el siguiente pedido:\n\n${lineas}\n\nEnvío: ${envio === 0 ? "Bonificado" : "$" + envio.toLocaleString("es-AR")}\n*Total: $${totalFinal.toLocaleString("es-AR")}*\n\nDatos de entrega:\nNombre: ${form.nombre}\nDirección: ${form.direccion}, ${form.ciudad}${extra}`;
  };
  ...
  const handleSubmit = () => {
    if (payMethod === "mp") handleMP();
    else if (payMethod === "wa") handleWhatsApp();
    else handleEfectivo();
  };
      ...
      <div className="min-h-screen bg-[#fafafa]">
        ...
        <div className="min-h-screen flex items-center justify-center px-6">
          <div className="text-center space-y-8">
            <p className="font-display text-4xl font-light italic tracking-tight">el archivo está vacío</p>
        ...
        <div className="max-w-[1300px] mx-auto">
          ...
          <header className="mb-14">
            ...
            <h1 className="font-display text-4xl md:text-5xl tracking-tight font-light">
              Finalizar <span className="italic text-[#0a0a0a]/30">archivo</span>
            </h1>
          </header>
            ...
            <div className="lg:col-span-7 space-y-12">
              ...
              <section>
                <div className="flex items-center gap-4 mb-8">
                  <div className="w-7 h-7 rounded-full border border-[#0a0a0a] flex items-center justify-center text-[9px] font-bold">1</div>
                  <h3 className="text-[10px] uppercase tracking-[0.35em] font-semibold">Información de Entrega</h3>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  {fields.map(({ key, label, placeholder, span }) => (
                    <div key={key} className={span ? "md:col-span-2" : ""}>
                      <label className="text-[8px] uppercase tracking-[0.3em] text-[#0a0a0a]/35 block mb-2 font-semibold">{label}</label>
                      <input
              ...
              <section className="pt-8 border-t border-[#0a0a0a]/6">
                <div className="flex items-center gap-4 mb-8">
                  <div className="w-7 h-7 rounded-full border border-[#0a0a0a] flex items-center justify-center text-[9px] font-bold">2</div>
                  <h3 className="text-[10px] uppercase tracking-[0.35em] font-semibold">Método de Pago</h3>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <button onClick={() => setPayMethod("mp")}
                    ...
                    <span className="text-[10px] font-semibold uppercase tracking-[0.2em] mb-0.5">Mercado Pago</span>
                    <span className="text-[10px] text-[#0a0a0a]/40 leading-relaxed">Tarjetas, débito, Cuenta DNI</span>
                  </button>

                  {/* Efectivo */}
                  <button onClick={() => setPayMethod("efectivo")}
                    className={`flex flex-col items-start p-5 border text-left transition-all ${
                      payMethod === "efectivo" ? "border-[#0a0a0a] bg-[#f2f2f0]" : "border-[#0a0a0a]/10 hover:border-[#0a0a0a]/30"
                    }`}>
                    <Banknote size={18} className="mb-3" strokeWidth={1.3} />
                    <span className="text-[10px] font-semibold uppercase tracking-[0.2em] mb-0.5">Efectivo</span>
                    <span className="text-[10px] text-[#0a0a0a]/40 leading-relaxed">Coordinamos entrega por WhatsApp</span>
                  </button>

                  {/* WhatsApp */}
                  <button onClick={() => setPayMethod("wa")}
                    className={`flex flex-col items-start p-5 border text-left transition-all ${
                    ...
                    <span className="text-[10px] font-semibold uppercase tracking-[0.2em] mb-0.5">WhatsApp</span>
                    <span className="text-[10px] text-[#0a0a0a]/40 leading-relaxed">Coordinamos todo por chat</span>
                  </button>
                </div>
                ...
                {payMethod === "efectivo" && (
                  <motion.div
                    ...
                    className="mt-4 p-4 bg-[#f5f5f3] border border-[#0a0a0a]/8 text-[11px] text-[#0a0a0a]/55 leading-relaxed"
                  >
                    Al confirmar te vamos a escribir por WhatsApp para coordinar el lugar y horario de entrega o retiro. Sin costo adicional.
                  </motion.div>
                )}
              </section>
            ...
            <div className="lg:col-span-5">
              <div className="bg-[#f2f2f0] p-8 lg:p-10 sticky top-28">
                ...
                <div className="space-y-5 mb-10 max-h-[35vh] overflow-y-auto pr-2">
                  {items.map((item) => (
                    <div key={item.id} className="flex gap-4 items-start">
                      <div className="w-14 flex-shrink-0 overflow-hidden bg-[#e8e8e6]" style={{ height: "72px" }}>
                        <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="text-[10px] font-semibold uppercase tracking-tight leading-tight truncate">{item.name}</h4>
                        <p className="text-[9px] text-[#0a0a0a]/40 mt-0.5 uppercase tracking-widest">Cant: {item.qty}</p>
                      </div>
                ...
                <div className="space-y-3 border-t border-[#0a0a0a]/6 pt-6 mb-8">
                  ...
                  <div className="flex justify-between text-[10px] uppercase tracking-[0.2em] text-[#0a0a0a]/40 font-semibold">
                    <div className="flex items-center gap-2"><Truck size={12} strokeWidth={1.5} /><span>Envío</span></div>
                  ...
                  <div className="flex justify-between items-end pt-2">
                    ...
                    <span className="font-display text-3xl font-light tracking-tight">${totalFinal.toLocaleString("es-AR")}</span>
                  </div>
                </div>
                ...
                >
                  {loading ? "Procesando..." :
                    payMethod === "mp" ? "Proceder al Pago" :
                    payMethod === "efectivo" ? "Coordinar por WhatsApp" :
                    "Enviar Pedido por WhatsApp"}
                </button>

                <div className="mt-5 flex items-center justify-center gap-2 opacity-30">
                  <ShieldCheck size={13} strokeWidth={1.3} />
                  <span className="text-[8px] uppercase tracking-[0.3em] font-medium">Compra segura</span>
                </div>
              </div>
            </div>
